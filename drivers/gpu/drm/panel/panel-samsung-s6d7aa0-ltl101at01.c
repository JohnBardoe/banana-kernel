...
static inline struct s6d7aa0_ltl101at01 *to_s6d7aa0_ltl101at01(struct drm_panel *panel)
{
...
static int s6d7aa0_ltl101at01_enable(struct drm_panel *panel)
{
	struct s6d7aa0_ltl101at01 *ctx = to_s6d7aa0_ltl101at01(panel);
	int ret;
...
static int s6d7aa0_ltl101at01_disable(struct drm_panel *panel)
{
	struct s6d7aa0_ltl101at01 *ctx = to_s6d7aa0_ltl101at01(panel);
	int ret;
...
static int s6d7aa0_ltl101at01_get_modes(struct drm_panel *panel)
{
	...
	mode = drm_mode_duplicate(panel->drm, &s6d7aa0_ltl101at01_mode);
	if (!mode)
		return -ENOMEM;
...
static int s6d7aa0_ltl101at01_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->backlight = s6d7aa0_ltl101at01_create_backlight(dsi);
	...
	ctx->panel.dev = dev;
	ctx->panel.funcs = &s6d7aa0_ltl101at01_panel_funcs;
