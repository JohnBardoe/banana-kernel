...
static int smb1360_enable_fg_access(struct smb1360_battery *battery)
{
	...
	if (ret == 0) {
		...
		dev_err(battery->dev, "enable FG access timed out\n");
...
err:
	dev_err(battery->dev, "failed to enable fg access: %d\n", ret);
	return ret;
...
static int smb1360_force_fg_reset(struct smb1360_battery *battery)
{
	...
	dev_dbg(battery->dev, "forcing FG reset!\n");
...
static int smb1360_fg_reset(struct smb1360_battery *battery)
{
	...
	dev_dbg(battery->dev, "FG reset: predicted: %d, now: %d, delta: %d, threshold: %d",
...
{
	int ret, i, timeout;
	u8 loaded_profile, new_profile, bid_mask;
	...
	ret = device_property_read_u32(battery->dev, "qcom,profile-a-rid-kohm",
				       &profile_rid[0]);
	if (ret < 0)
		return ret;

	ret = device_property_read_u32(battery->dev, "qcom,profile-b-rid-kohm",
				       &profile_rid[1]);
	if (ret < 0)
		return ret;

	rid10 = battery->connected_rid / 10;
	for (i = 0; i < BATTERY_PROFILE_MAX; i++) {
		if (abs(profile_rid[i] - battery->connected_rid) < rid10)
			break;
	}
	dev_dbg(battery->dev, "battery profile: loaded %d, matched: %d\n",
		loaded_profile, i);
...
static int smb1360_update_bounds(struct smb1360_battery *battery)
{
	int ret;
	u32 val;
...
static int smb1360_check_cycle_stretch(struct smb1360_battery *battery)
{
	int ret;
	u32 val;

	ret = regmap_read(battery->regmap, STATUS_4_REG, &val);
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(battery->regmap, CMD_I2C_REG, CYCLE_STRETCH_CLEAR_BIT,
				 CYCLE_STRETCH_CLEAR_BIT);
	if (ret)
		dev_err(battery->dev, "unable to clear cycle stretch: %d\n", ret);
...
static int smb1360_delayed_hw_init(struct smb1360_battery *battery)
{
	...
	if (ret) {
		dev_err(battery->dev, "couldn't reconfigure OTP: %d\n", ret);
		return ret;
	}

	ret = smb1360_fg_reset(battery);
	if (ret)
		dev_err(battery->dev, "smb1360_fg_reset failed: %d\n", ret);

	ret = smb1360_update_bounds(battery);
	if (ret) {
		dev_err(battery->dev, "couldn't configure SOC/voltage bounds: %d\n", ret);
		return ret;
	}

	ret = smb1360_fg_config(battery);
	if (ret) {
		dev_err(battery->dev, "couldn't configure FG: %d\n", ret);
		return ret;
	}

	ret = smb1360_check_cycle_stretch(battery);
	if (ret) {
		dev_err(battery->dev, "Unable to check cycle-stretch\n");
		return ret;
	}
...
static int smb1360_parse_batt_id(struct smb1360_battery *battery)
{
	...
	dev_dbg(battery->dev, "batt-id voltage: %lld, connected_rid: %d\n",
		batt_id_uv, battery->connected_rid);
...
static int smb1360_parse_properties(struct smb1360_battery *battery)
{
	...
	battery->rsense_10mohm = device_property_read_bool(dev, "qcom,rsense-10mohm");
...
static int smb1360_probe(struct i2c_client *client)
{
	...
	dev_dbg(battery->dev, "device revision: %d\n", battery->revision);
	...
	ret = regmap_bulk_read(battery->regmap, IRQ_REG, battery->irqstat, IRQ_COUNT);
	if (ret < 0) {
