...
static struct private_data *cpufreq_dt_data;
...
{
	struct private_data *priv = cpufreq_dt_data;
...
static int dt_cpufreq_init_cpu(struct device *dev, int cpu)
{
	...
	priv->next = cpufreq_dt_data;
	cpufreq_dt_data = priv;
	...
	if (fallback) {
		...
		/*
		 * OPP tables are initialized only for current cpu, do it for
		 * others as well.
		 */
